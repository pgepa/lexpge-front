...
import { Eye, SquareArrowOutUpRight, SearchX, PencilLine } from 'lucide-react';
import {
    Pagination,
    PaginationContent,
    PaginationItem,
...
const ResultsList: React.FC = () => {
    ...
    const fetchResults = async (pagina: number) => {
        ...
        if (query.tipo && query.tipo !== 'todos') queryParams.tipo = query.tipo;
        if (query.situacao && query.situacao !== 'todas') queryParams.situacao = query.situacao;
        if (query.fonte && query.fonte !== 'todas') queryParams.fonte = query.fonte;
        if (query.origem && query.origem.trim() !== '') queryParams.origem = query.origem.trim();
