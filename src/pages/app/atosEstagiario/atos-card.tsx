...
import { Eye, PencilLine, SquareArrowOutUpRight } from 'lucide-react';
import {
  Pagination,
  PaginationContent,
  PaginationItem,
...
export const AtosCardEstagiario = () => {
  const [atos, setAtos] = useState<AtoCardEstagiario[]>([]);
  ...
  async function loadAtosCardEstagiario(pagina = 1, filters = {}) {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    loadAtosCardEstagiario(currentPage, filters);
  }, [currentPage, filters]);
  ...
  const handleTextoIntegralClick = (ato: AtoCardEstagiario) => {
    const textoIntegralUrl = `/#/estagiario/texto-integral/${ato.id}`;
    const link = document.createElement('a');
  ...
  const handleEditClick = (ato: AtoCardEstagiario) => {
